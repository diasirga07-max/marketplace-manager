...
function counts(groups){
  const r={};
  for(const g of GROUPS){const a=groups[g]||[];r[g]={rows:a.length,units:a.reduce((s,x)=>s+(Number(x.qty)||0),0)}}
  return r;
}
...
async function sync(force=false){
  ...
  syncing=(async()=>{
    ...
    badge('Excel: '+(j.updatedAt||'обновлён')+' · К '+c['Курдай'].rows+' / WB '+c['WB'].rows+' / А '+c['Алматы'].rows,'ok');
    return j;
  })().catch(e=>{
