...
const VERSION = 'Vercel WB→Sheets V2 curl';
const WB_BATCH = 100;
const WB_PARALLEL = 6;
let tokenCache = null;
...
function parseProduct(p) {
  ...
  for (const size of Array.isArray(p?.sizes) ? p.sizes : []) {
    ...
    if (!hours) {
      const times = (Array.isArray(size?.stocks) ? size.stocks : []).filter(x => Number(x?.qty || 0) > 0).map(x => Math.max(0, Number(x?.time1 || 0)) + Math.max(0, Number(x?.time2 || 0))).filter(Boolean);
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (mode === 'health') {
      ...
      return send(res, 200, { ok: true, version: VERSION, googleSheets: true, sampleRows: rows.length, destination: WB_DESTINATION });
    }
    if (mode === 'probe') {
      const id = Number(req.query?.nm || 598732175);
      const map = await wbBatch([id]);
      return send(res, 200, { ok: true, version: VERSION, destination: WB_DESTINATION, nm: id, snapshot: map.get(id) || null });
    }
    ...
    const out = rows.map((r, idx) => {
      ...
      return [
        ...
        `Обновлено ${VERSION}; товар=${p.product}; логистика=${p.logistics}; dest=${WB_DESTINATION}; ${ts}`
      ];
    });

    await sheetsPut(`U2:Y${out.length + 1}`, out);
    return send(res, 200, {
      ok: true, version: VERSION, destination: WB_DESTINATION,
    ...
    console.error('wb-sheets-sync-v2 failed', e);
