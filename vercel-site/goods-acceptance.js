...
function stopAcceptance(){stopRequested=true;window.GB_GOODS_ACCEPTANCE_STOPPED=true;showMsg('Остановка включена. Уже отправленный запрос может завершиться, следующие заказы не будут отправлены.','warn');const b=$('#gaStop');if(b){b.textContent='⛔ Остановлено';b.disabled=true}}
function showMsg(text,type='warn'){const m=$('#gaMsg');if(!m)return;m.textContent=text;m.className='gaMsg show '+type}
function clearMsg(){const m=$('#gaMsg');if(!m)return;m.className='gaMsg';m.textContent=''}
...
build();nav();let tries=0;const timer=setInterval(()=>{if(nav()||++tries>60)clearInterval(timer)},500);
})();
